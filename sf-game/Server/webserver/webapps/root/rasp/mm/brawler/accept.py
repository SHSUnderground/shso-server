#
# handle accept invite to mission
...
class accept(HttpServlet):
	...
	def doPost(self, request, response):
		...
		for name in request.getParameterNames():
			...
			if name == "invitation_id":   # invitees list
				invitation_id = request.getParameter(name)
			if name == "accept":   # invitees list
				accept = request.getParameter(name)
			if name == "inviter_id":   # invitees list
				inviter_id = request.getParameter(name)
		...
		arr = ArrayList()
		arr.add("get_game_room_accept")  # handler to call in target extension
		arr.add(session_key)
		arr.add(mission)  # should be mission ID
		arr.add(game)
		arr.add(user)
		#arr.add(invitees)
		arr.add(invitation_id)
		arr.add(accept)
		arr.add(inviter_id)
